...
class MovingSpriteDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for i in range(self.batch_size):
            traj = self._generator.gen_trajectory()
            if i == 0:
                data_dict.images = np.expand_dims(traj.images[:, None].astype(np.float32) / (255./2) - 1.0, axis=0)
                data_dict.states = np.expand_dims(traj.states, axis=0)
                data_dict.shape_idxs = np.expand_dims(traj.shape_idxs, axis=0)
                data_dict.rewards = traj.rewards
                for key in data_dict.rewards.keys():
                    data_dict.rewards[key] = np.expand_dims(data_dict.rewards[key], axis=0)

            else:
                data_dict.images = np.concatenate((np.expand_dims(traj.images[:, None].astype(np.float32) / (255./2) - 1.0, axis=0), data_dict.images), axis=0)
                data_dict.states = np.concatenate((np.expand_dims(traj.states, axis=0), data_dict.states), axis=0)
                data_dict.shape_idxs = np.concatenate((np.expand_dims(traj.shape_idxs, axis=0), data_dict.shape_idxs), axis=0)
                for key in data_dict.rewards.keys():
                    data_dict.rewards[key] = np.concatenate((np.expand_dims(traj.rewards[key], axis=0), data_dict.rewards[key]), axis=0)

        # print('data_dict.images.shape: ', data_dict.images.shape)
        data_dict.images = torch.from_numpy(data_dict.images)
        data_dict.states = torch.from_numpy(data_dict.states)
        data_dict.shape_idxs = torch.from_numpy(data_dict.shape_idxs)

        for key in data_dict.rewards.keys():
            data_dict.rewards[key] = torch.from_numpy(data_dict.rewards[key])
            # print('data_dict.rewards[key].shape: ', data_dict.rewards[key].shape)

        return data_dict


# Interface for other instatiable generator classes
class MovingSpritesGenerator:
    ...
    def _sample_shapes(self):
        ...
        # Pay attention to this base class
...
if __name__ == '__main__':
    ...
    testing_dataset = MovingSpriteDataset(spec)
    print('printed here: ', testing_dataset.__getitem__(0))
